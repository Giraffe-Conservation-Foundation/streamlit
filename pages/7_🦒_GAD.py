...
def process_data(df, species_filter=None, subspecies_filter=None, country_filter=None, region0_filter=None):
    ...
    # Summarize at ISO (country) level - for records with empty Region0
    iso_data = (latest_data[(latest_data['Site'].isna() | (latest_data['Site'] == '')) & 
                            (latest_data['Region1'].isna() | (latest_data['Region1'] == '')) &
                            (latest_data['Region0'].isna() | (latest_data['Region0'] == ''))]
                .groupby(['Country', 'Species', 'Subspecies'])
                .agg({
                    'Estimate': 'sum',
                    'Lower': 'sum',
                    'Upper': 'sum',
                ...
                .reset_index())
    # Add empty columns for Region0, Region1, Site
    iso_data['Region0'] = ''
    iso_data['Region1'] = ''
    iso_data['Site'] = ''
    
    # Remove duplicates - use most granular data available
    # If region1 data exists, don't use region0 for same location
    region1_keys = region1_data[['Country', 'Species', 'Subspecies', 'Region0']].drop_duplicates()
    region0_data = region0_data.merge(
        region1_keys,
        on=['Country', 'Species', 'Subspecies', 'Region0'],
        how='left',
        indicator=True
    )
    region0_data = region0_data[region0_data['_merge'] == 'left_only'].drop('_merge', axis=1)
    
    # If region0 data exists, don't use ISO for same location
    region0_keys = region0_data[['Country', 'Species', 'Subspecies']].drop_duplicates()
    iso_data = iso_data.merge(
        region0_keys,
        on=['Country', 'Species', 'Subspecies'],
        how='left',
        indicator=True
    )
    iso_data = iso_data[iso_data['_merge'] == 'left_only'].drop('_merge', axis=1)
    
    # Combine all data
    combined = pd.concat([site_data, region1_data, region0_data, iso_data], ignore_index=True)
...
max_year_filter = st.sidebar.slider(
    "Maximum Survey Year",
    min_value=min_year,
    max_value=max_year,
    value=max_year,
...
species_filter = st.sidebar.multiselect("Species", species_options, max_selections=None)
...
subspecies_filter = st.sidebar.multiselect("Subspecies", subspecies_options, max_selections=None)
...
country_filter = st.sidebar.multiselect("Country", country_options, max_selections=None)
...
region0_filter = st.sidebar.multiselect("Region", region0_options, max_selections=None)
