...
if tool_choice == "🏠 Home":
    ...
    try:
        # Try multiple possible locations for the wildbook app
        possible_paths = [
            current_dir / "wildbook_id_generator" / "app.py",
            current_dir / "wildbook_app.py",  # Fallback to root directory
            Path("wildbook_id_generator/app.py"),  # Relative path
            Path("wildbook_app.py")  # Root fallback
        ]
        ...
        for path in possible_paths:
            if path.exists():
                app_file = path
                wildbook_path = path.parent if path.name == "app.py" else current_dir
                break
        ...
        if app_file and app_file.exists():
            ...
            try:
                ...
                # Change to the app directory if it's in a subdirectory
                if wildbook_path != current_dir:
            ...
            st.info("Checked locations: wildbook_id_generator/app.py, wildbook_app.py")
            st.info("Available files in repository:")
            ...
            try:
                ...
                st.info("Could not list repository files")
    ...
    try:
        # Try multiple possible locations for the NANW app
        possible_paths = [
            current_dir / "nanw_dashboard" / "app.py",
            current_dir / "nanw_app.py",  # Fallback to root directory
            Path("nanw_dashboard/app.py"),  # Relative path
            Path("nanw_app.py")  # Root fallback
        ]
        ...
        for path in possible_paths:
            if path.exists():
                app_file = path
                nanw_path = path.parent if path.name == "app.py" else current_dir
                break
        ...
        if app_file and app_file.exists():
            ...
            st.info("Checked locations: nanw_dashboard/app.py, nanw_app.py")
            st.info("Available files in repository:")
            # List available Python files for debugging
            try:
                import subprocess
