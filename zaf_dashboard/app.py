...
def main():
    ...
    # Force deployment update - timestamp: Sep 3, 2025 - FIXED BLANK PAGE ISSUE
    # ZAF Dashboard - Complete implementation with satellite mapping - RESTRUCTURED FOR LAUNCHER
    ...
    if not st.session_state["authenticated"]:
        st.title("🦒 Login to ZAF EarthRanger Dashboard")
    ...
    def get_active_sources():
        """Get active tracking sources for ZAF giraffes"""
        er = EarthRangerIO(
    ...
    @st.cache_data(ttl=3600)
    def load_data():
        """Load ZAF giraffe survey encounter data"""
        er = EarthRangerIO(
            server="https://twiga.pamdas.org",
            username=username,
            password=password
        ...
        # ZAF-specific event parameters
        event_cat = "monitoring_zaf"
        ...
        try:
            ...
            if events.empty:
                ...
                st.info("💡 Please verify the event category and type names are correct for ZAF")
                return pd.DataFrame()
            ...
            st.error(f"❌ Error loading ZAF data: {str(e)}")
    ...
    if df.empty:
        ...
        st.info("""
        **Possible issues:**
        1. Event category 'monitoring_zaf' may not exist
        2. Event type 'giraffe_survey_encounter_zaf' may not exist  
        3. No data in the specified date range
        4. Subject group IDs need to be updated
        
        **Next steps:**
        - Verify the correct event category and type names for ZAF
        - Update the subject group IDs in the code
        - Check if there's data in the specified date range
        """)
        st.stop()

    # Rename columns (similar to NANW but adapted for ZAF structure)
    rename_map = {
    ...
    #### Simplified - AAG section removed as not applicable for ZAF
